...
const zhCopy = {
  ...
  oauthSection: {
    ...
    claudeRetiredBody:
      '此登录方式已从 Maka 移除。这条连接会保留在列表中，但无法再登录或用于对话。改用 Anthropic API Key 连接即可继续使用 Claude 模型；在「模型」页删除这条连接，即可清除本机保存的登录凭据。',
    codexDescription: 'ChatGPT Plus / Pro 订阅账号登录。', xaiDescription: 'SuperGrok / X Premium 账号登录。',
    copilotDescription: '导入兼容 GitHub 凭据连接 Copilot 订阅。', serviceUnavailable: '登录服务暂时不可用，请检查网络后重试。',
    aria: 'OAuth 登录',
    staleState: 'OAuth 登录状态暂时没刷新成功，已保留上一次状态。', claudeSubtitle: '此登录方式已停用。',
...
const enCopy: ProviderSettingsCopy = {
  ...
  oauthSection: {
    ...
    claudeRetiredBody:
      'This sign-in path has been removed from Maka. This connection stays listed but can no longer be signed into or used for a conversation. Add an Anthropic API key connection to keep using Claude models; deleting this connection under Models clears the sign-in credentials stored on this machine.',
    codexDescription: 'Sign in with a ChatGPT Plus / Pro subscription.', xaiDescription: 'Sign in with SuperGrok or X Premium.',
    ...
    aria: 'OAuth sign-in',
    staleState: 'OAuth sign-in status could not be refreshed. The last known state is preserved. ', claudeSubtitle: 'This sign-in path is retired.',
